...
/**
 * @returns `hex value` for a given rgb value
 * @param rgb `string` provide valid rgb value. ex: `rgb(86,86,86)`
 */
export function RGBtoHEX(rgb: string) {
  const isValidRGB = rgbTest(rgb);
  if (!isValidRGB) {
...
/**
 * @returns `rgb value` for a given hex value
 * @param hex `string` provide valide hex value ex: `567`, `#567`, `#565656`, `565656`
 */
export function HEXtoRGB(hex: string) {
  let modifyHex: string = hex;
  const rgbArr = [];
  const isValidHex = hexTest(hex);
  if (!isValidHex) {
...
function hexTest(hexValue: string) {
  const hexRegex = /^[#]*([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/i;
  return hexRegex.test(hexValue);
}

function rgbTest(rgbValue: string) {
...
// export function getFullDetailsAboutColor(type: string, color: string) {
//   return new Promise(async (res, rej) => {
//     const url = 'http://thecolorapi.com/id?';
//     switch (type) {
//       case 'hex':
//         const isValideHex = hexTest(color);
//         if (isValideHex) {
//           throw new Error('not a valid hex value');
//         } else {
//           const request = new Request(`${url}hex=${color}`, {
//             cache: 'no-cache',
...
//           });
//           try {
//             const colorRes = await fetch(request);
//             if (colorRes.ok) {
//               res(colorRes.);
//             } else {
//               rej('Give valid hex value');
//             }
//           } catch (error) {
//             rej(error);
//           }
